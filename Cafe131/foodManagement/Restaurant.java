...
public class Restaurant {
	...
	public Restaurant(String nameIn, int startingCash) {
		name=nameIn;       // name of food
		cash=startingCash; //cash amount
		menu= new SortedListOfImmutables(); //creates Immutables for menu
		//creates Immutables for inventory
	...
	public SortedListOfImmutables getInventory() {
		//Creates Immutables of inventory
	...
	public boolean checkIfInInventory(Entree entree) {
		//check inventory and whether food is available
	...
	public boolean addShipmentToInventory(SortedListOfImmutables list) {
		if(list.getWholesaleCost()>cash) {/* When list is greater than cash*/
			return false;
		}
		inventory.add(list);//adds item to inventory if your able 
		cash-=list.getWholesaleCost();//subtracts cash after purchase
		return true;
	...
	public boolean placeOrder(Entree entree) {
		/*If the food is in the inventory then item gets ordered*/
		if (inventory.checkAvailability(entree.getFoodList())) {
			cash+=entree.getRetailValue(); //profit for the cafe
			//remove amount being ordered
